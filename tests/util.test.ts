...
describe('Util', () => {
  ...
  describe('"$" function', () => {
    before(() => {
      document = new JSDOM(
        `<body><div class="foo"><p class="bar">Foobar</p></div></body>`,
      ).window.document
    })
    it('should exist', () => {
      expect(Util.$).exist
    ...
    context('when selector exists inside the DOM', () => {
      ...
      it('should return value which has the expected HTML element', () => {
        expect(Util.$('.foo', document)[0].innerHTML).to.be.equal(
          `<p class="bar">Foobar</p>`,
        )
      })
    })
  ...
  describe('"strPad" function', () => {
    ...
    context('when invoked with "STR_PAD_RIGHT" type', () => {
      it('should return a right-padded string', () => {
        expect(
          Util.strPad('bar', 9, 'foo', Util.PadType.STR_PAD_RIGHT),
        ).to.equal('barfoofoo')
      })
    })
    context('when invoked with "STR_PAD_LEFT" type', () => {
      it('should return a left-padded string', () => {
        expect(
          Util.strPad('bar', 9, 'foo', Util.PadType.STR_PAD_LEFT),
        ).to.equal('foofoobar')
      })
    })
    context('when invoked with "STR_PAD_BOTH" type', () => {
      it('should return a both-padded string', () => {
        expect(
          Util.strPad('bar', 9, 'foo', Util.PadType.STR_PAD_BOTH),
        ).to.equal('foobarfoo')
      })
    })
  })
